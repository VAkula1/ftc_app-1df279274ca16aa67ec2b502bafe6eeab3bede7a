...
public class KataPositionDetector extends DogeCVDetector {
     ...
     * Simple constructor
     */
    public KataPositionDetector() {
        super();
        detectorName = "KataPositionDetector"; // Set the detector name
    }
    ...
    public Mat process(Mat input) {
        ...
        if(bestRect != null){
            ...
            // Draw center point
            Imgproc.circle(displayMat, new Point( yPos, bestRect.y + (bestRect.height / 2)), 5, new Scalar(0,255,0),2);

            // Check if the mineral is aligned
            if(yPos < alignXMax && yPos > alignXMin){
                aligned = true;
            }else{
                aligned = false;
            }
            ...
            // Draw Current X
            Imgproc.putText(displayMat,"Current X: " + bestRect.y,new Point(getAdjustedSize().height -10, 10),0,0.5, new Scalar(255,255,255),1);
            found = true;
        }else{
            found = false;
            aligned = false;
        ...
        if(debugAlignment){
            ...
            if(isFound()){
                Imgproc.line(displayMat,new Point(goldYPos, getAdjustedSize().height), new Point(goldYPos, getAdjustedSize().height - 30),new Scalar(255,255,0), 2);
            }
        ...
        //Print result
        Imgproc.putText(displayMat,"Result: " + aligned,new Point(getAdjustedSize().height - 30,10),0,1, new Scalar(255,255,0),1);
